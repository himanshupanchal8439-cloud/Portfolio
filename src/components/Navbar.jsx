...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-4xl">
      <nav className="glass-nav rounded-2xl px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
        <a href="#home" className="text-lg sm:text-xl font-bold tracking-tighter flex items-center gap-2 text-white" data-cursor>
      ...
      <div className={`lg:hidden ${open ? 'flex' : 'hidden'} glass-nav mt-3 rounded-2xl flex-col gap-1 px-6 py-4 text-gray-300`}>
        {LINKS.map((l) => (
